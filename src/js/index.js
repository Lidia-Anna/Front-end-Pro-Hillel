...
(() => {
  ...
  const generateList = (listItems, listType = LIST_TYPES.UL) => {
    if (!Object.values(LIST_TYPES).includes(listType)) throw new Error('listType arguments must be a List type');
    if (!listItems || !Array.isArray(listItems)) throw new Error('Array arguments must be a List array');
    if (!listItems.length) throw new Error("array length shouldn't be a 0");
